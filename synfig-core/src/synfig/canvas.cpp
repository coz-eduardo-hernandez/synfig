...
//		throw std::runtime_error("You cannot add a ValueNode to an inline Canvas");
...
//	int i(x->count());
	...
	/*if(x->count()!=i+1)
	{
		...
		return;
		//throw std::runtime_error("Canvas Insertion Failed");
	}*/
...
Canvas::clone(const GUID& deriv_guid, bool for_export)const
{
	...
	else
	{
		...
		throw std::runtime_error("Cloning of non-inline canvases is not yet supported");
